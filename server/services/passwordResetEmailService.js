...
const buildPasswordResetEmail = ({ resetUrl, userName }) => {
  ...
  const subject = `${COMPANY_NAME}: slaptažodžio atkūrimas`;
  const text = [
    COMPANY_NAME,
    "",
    `Sveiki, ${userName?.trim() || "nary"},`,
    "",
    "Gavome prašymą atkurti Stilloak Studio paskyros slaptažodį.",
    "Nuoroda galioja trumpai ir gali būti panaudota tik vieną kartą:",
    resetUrl,
    "",
    "Jei šio prašymo neteikėte, galite ignoruoti šį laišką.",
  ].join("\n");
  const html = `
    <div style="margin:0;padding:24px;background:#f8f4ee;font-family:Arial,sans-serif;color:#2b241d;">
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #ece3d7;border-radius:18px;padding:32px;">
        <p style="margin:0 0 12px 0;font-size:12px;letter-spacing:0.2em;text-transform:uppercase;color:#8a6c46;">
        ...
        <p style="margin:0 0 18px 0;font-size:15px;line-height:1.7;color:#6d5c4c;">
          Gavome prašymą atkurti Stilloak Studio paskyros slaptažodį. Ši nuoroda galioja trumpai ir gali būti panaudota tik vieną kartą.
        </p>
...
const sendPasswordResetThroughSmtp = async ({ to, email }) => {
...
const sendPasswordResetEmail = async ({ to, resetUrl, userName }) => {
  const email = buildPasswordResetEmail({ resetUrl, userName });
  ...
  return sendPasswordResetThroughSmtp({ to, email });
};
