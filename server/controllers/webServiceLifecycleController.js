...
const resendAdminWebServiceTestContract = async (req, res) => {
  ...
  request.contactHistory.push({ type: "email", note: "Pakartotinai išsiųsta testinė projekto sutartis.", happenedAt: new Date(), actorName: req.user?.name || "Administratorius", actorEmail: req.user?.email || "" });
  await request.save();
