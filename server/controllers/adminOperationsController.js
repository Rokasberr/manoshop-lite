...
const getAdminOperations = async (_req, res) => {
  const [events, failedDeliveries, latestBackup] = await Promise.all([
  ...
  res.json({ backup: { enabled: String(process.env.DATABASE_BACKUP_ENABLED || "false").toLowerCase() === "true", configured: isDatabaseBackupConfigured(), lastSuccessAt: latestBackup?.createdAt || null }, events, failedDeliveries });
};
